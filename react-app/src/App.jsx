http://localhost:5173import { useEffect, useMemo, useRef, useState } from 'react'
import {
  BrowserRouter,
  Link,
  NavLink,
...
function SignupPage() {
  ...
  // Tutoring slots data
  const slots = [
    { date: '04/01/2026', time: '7:00pm - 8:00pm', day: 'Wednesday', tutor: 'Abhiram M.', filled: 0, capacity: 3 },
    { date: '04/02/2026', time: '7:00pm - 8:00pm', day: 'Thursday', tutor: 'Neha M.', filled: 0, capacity: 3 },
    { date: '04/04/2026', time: '11:00am - 12:00pm', day: 'Saturday', tutor: 'Abhiram M.', filled: 0, capacity: 3 },
    { date: '04/05/2026', time: '11:00am - 12:00pm', day: 'Sunday', tutor: 'Neha M.', filled: 0, capacity: 3 },
    { date: '04/06/2026', time: '7:00pm - 8:00pm', day: 'Monday', tutor: 'Nandita S.', filled: 0, capacity: 3 },
    { date: '04/07/2026', time: '7:30pm - 8:30pm', day: 'Tuesday', tutor: 'Samhithaa S.', filled: 0, capacity: 3 },
    { date: '04/08/2026', time: '7:00pm - 8:00pm', day: 'Wednesday', tutor: 'Abhiram M.', filled: 0, capacity: 3 },
    { date: '04/09/2026', time: '7:00pm - 8:00pm', day: 'Thursday', tutor: 'Neha M.', filled: 0, capacity: 3 },
    { date: '04/11/2026', time: '11:00am - 12:00pm', day: 'Saturday', tutor: 'Abhiram M.', filled: 1, capacity: 3 },
    { date: '04/12/2026', time: '11:00am - 12:00pm', day: 'Sunday', tutor: 'Neha M.', filled: 1, capacity: 3 },
  ]
  ...
  function handleSubmit(e) {
    ...
    // Validation
    if (!selectedSlot) {
      setError('Please select a tutoring slot.')
      return
    }
    ...
    if (!formData.goals.trim()) {
      setError('Please describe the tutoring goals.')
      return
    }

    // Here you would submit to your backend
    console.log('Signup submitted:', { selectedSlot, ...formData })
    setSubmitted(true)
    ...
    return (
      <div className="page-shell">
        <section className="panel hero">
          ...
          <p className="hero-copy">
            Your tutoring session has been scheduled for <strong>{selectedSlot.date}</strong> at{' '}
            <strong>{selectedSlot.time}</strong> with <strong>{selectedSlot.tutor}</strong>.
          </p>
          <p style={{ marginTop: '1rem', fontSize: '0.95rem', color: 'var(--text-muted)' }}>
            A confirmation email has been sent to {formData.parentEmail}. We look forward to seeing you!
          </p>
        </section>
      </div>
    )
  ...
  return (
    <div className="page-shell">
      <section className="panel hero">
        <p className="eyebrow">Schedule Your Session</p>
        <h1>Tutoring Session Signup</h1>
        <p className="hero-copy">Choose an available slot and complete your information below.</p>
      </section>
        ...
        <div style={{ display: 'grid', gap: '0.8rem', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))' }}>
          {slots.map((slot, idx) => {
            ...
            return (
              ...
              >
                <strong>{slot.date}</strong> · {slot.day}
                <div style={{ fontSize: '0.9rem', marginTop: '0.4rem', color: 'var(--text-muted)' }}>
                  {slot.time}
                </div>
                <div style={{ fontSize: '0.85rem', marginTop: '0.4rem' }}>
                  <strong>{slot.tutor}</strong>
                </div>
                <div style={{ fontSize: '0.8rem', marginTop: '0.3rem', color: 'var(--text-muted)' }}>
                  {slot.filled} of {slot.capacity} filled {!isAvailable && '(Full)'}
                </div>
              </button>
            )
          })}
      ...
      <section className="panel">
        ...
        {error && (
          <div style={{ padding: '0.8rem', background: '#fee', color: '#c33', borderRadius: '0.4rem', marginBottom: '1rem' }}>
            {error}
          </div>
        )}

        {selectedSlot && (
          <div style={{ padding: '0.8rem', background: 'rgba(90, 87, 230, 0.05)', borderRadius: '0.4rem', marginBottom: '1rem' }}>
            ✓ Selected: <strong>{selectedSlot.date}</strong> at <strong>{selectedSlot.time}</strong> with <strong>{selectedSlot.tutor}</strong>
          </div>
        )}
        ...
        <form onSubmit={handleSubmit}>
          <label style={{ display: 'block', marginBottom: '1rem' }}>
            ...
            <input
              ...
              required
              style={{ marginTop: '0.3rem', width: '100%', padding: '0.6rem', border: '1px solid var(--border)', borderRadius: '0.4rem' }}
            />
          </label>
          ...
          <label style={{ display: 'block', marginBottom: '1rem' }}>
            Tutoring Goals / Session Description *
            <textarea
              name="goals"
              value={formData.goals}
              onChange={handleFormChange}
              placeholder="What would you like to accomplish? What topics would you like to work on?"
              rows="4"
              required
              style={{ marginTop: '0.3rem', width: '100%', padding: '0.6rem', border: '1px solid var(--border)', borderRadius: '0.4rem' }}
            />
