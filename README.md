...
A React version has been started in `react-app/` with a redesigned UI and live TA/review loading.

1. Keep the mock API server running on port `9999`.
...
- React uses a Vite proxy for `/api/*` to `http://localhost:9999` in local development.
- The React app currently reads TA data from `react-app/public/data/tas.json`.
- Existing static pages are untouched and still available.
